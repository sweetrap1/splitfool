...
function updateSplitSummary() {
    ...
    // accurate ΓÇö a global query could pick up stale checked elements from old renders.
    ...
    try {
        ...
        splitContainer.querySelectorAll('.participant-input').forEach(input => {
            ...
            try {
                ...
                if (!cbEl) return; // Safety guard ΓÇö skip non-participant inputs
        ...
        if (currentSplitMode === 'paid_for') {
            ...
            // For equal mode, calculate properly and show ΓÇö no error check needed
            if (totalSelected > 0) {
    ...
    if (currentSplitMode === 'paid_for') {
        ...
        checkboxes.forEach(cb => {
            ...
            if (currentSplitMode === 'equal') {
                // Always compute fresh for equal mode ΓÇö don't rely on disabled DOM inputs
...
function calculateBalances(ignoreSettlements = false) {
    ...
    activeGroup.expenses.forEach(e => {
        ...
        e.participants.forEach(p => {
            ...
            // (e.g., 100/3 = 33.3333... ΓåÆ 33.33 so 3 participants sum to 99.99, not 100.00000001)
...
function renderSettleUp() {
    ...
    if (usedCurrencies.size === 0) {
        container.innerHTML = '<div class="card" style="text-align:center; padding: 1rem;">All settled up! ≡ƒÄë</div>';
        if (manualRateContainer) manualRateContainer.classList.add('hidden');
        return;
    }
    ...
    if (!isCombined) {
        ...
        container.innerHTML = finalHtml || '<div class="card" style="text-align:center; padding: 1rem;">All settled up! ≡ƒÄë</div>';
    } else {
